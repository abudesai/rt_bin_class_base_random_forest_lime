...
def explain():
    """Get local explanations on a single batch of data.
    We take data as a JSON object, convert it to a pandas data frame for internal use,
    and then convert the predictions back to JSON.
    """
    # Convert from CSV to pandas
    if flask.request.content_type == "application/json":
        req_data_dict = json.loads(flask.request.data.decode("utf-8"))
    ...
    # Do the prediction
    try:
        explanations = model_server.explain_local2(data)
